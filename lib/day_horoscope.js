...
module.exports = class DayHoroscope extends Model {
	constructor(db) {
		super(db, 'DayHoroscope');
	}

	normalize(data) {
		return helpers.normalizeDayHoroscope(data);
	}

	static createId() {
		return helpers.createDayHoroscopeId.apply(null, arguments);
	}
	 ...
	 */
	generate(phraseModel, data, options) {
		const self = this;
